...
$sql_ActeMaitrise = "SELECT role_resid 
FROM relation_actp_resid_med 
WHERE role_resid='Acte Maitrise' 
AND medecin_valid IS NOT NULL 
AND id_resid = (SELECT id FROM members WHERE username = '" . mysqli_real_escape_string($conn, $_SESSION['username']) . "')";
$result_ActeMaitrise = mysqli_query($conn, $sql_ActeMaitrise);


$sql_Operateuraide = "SELECT role_resid 
FROM relation_actp_resid_med 
WHERE role_resid='Operateur aide' 
AND medecin_valid IS NOT NULL 
AND id_resid = (SELECT id FROM members WHERE username = '" . mysqli_real_escape_string($conn, $_SESSION['username']) . "')";
$result_Operateuraide = mysqli_query($conn, $sql_Operateuraide);


$sql_AideOperateur = "SELECT role_resid 
FROM relation_actp_resid_med 
WHERE role_resid='Aide Operateur' 
AND medecin_valid IS NOT NULL 
AND id_resid = (SELECT id FROM members WHERE username = '" . mysqli_real_escape_string($conn, $_SESSION['username']) . "')";
$result_AideOperateur = mysqli_query($conn, $sql_AideOperateur);


$sql_Observateur = "SELECT role_resid 
FROM relation_actp_resid_med 
WHERE role_resid='Observateur' 
AND medecin_valid IS NOT NULL 
AND id_resid = (SELECT id FROM members WHERE username = '" . mysqli_real_escape_string($conn, $_SESSION['username']) . "')";
